...
function SelectTemas({ value, onChange }) {
    ...
    return (
        <FormControl fullWidth>
            <InputLabel id="temas-label" style={{color:'white'}}>Temas</InputLabel>
            <Select
                labelId="temas-label"
                id="temas-select"
                multiple
                ...
                renderValue={(selected) => (
                    <div style={{ display: 'flex', flexWrap: 'wrap'}}>
                        {selected.map((temaId) => (
                            <Chip key={temaId} label={temas.find((tema) => tema._id === temaId)?.titulo} style={{ margin: '2px', backgroundColor: 'skyblue'  }} />
                        ))}
                    </div>
                )}
            >
