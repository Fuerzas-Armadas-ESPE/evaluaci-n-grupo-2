...
function CursoList() {
    ...
    const loadCursos = async () => {
        try {
            ...
            const cursosConTemas = await Promise.all(data.map(async (curso) => {
                const temasPromises = curso.temas.map(async (temaId) => {
                    const temaResponse = await fetch(`http://localhost:3000/temas/${temaId}`);
                    if (!temaResponse.ok) {
                ...
                >
                    ...
                    >
                        <div style={{ color: 'white' }}>
                            ...
                            <Typography variant="body2">Creado el: {new Date(curso.createdAt).toLocaleString()}</Typography>
                        </div>
                        <div>
                            <Button
                                variant='contained'
